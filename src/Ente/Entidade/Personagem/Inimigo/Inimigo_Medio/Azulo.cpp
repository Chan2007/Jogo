...
namespace Personagens {
    Azulo::Azulo() : Inimigo(), tamanho(40) {
        ...
        caminhoArquivoSpriteMovimento = Encontrar_Caminho::acharDiretorio_Arquivo("assets/sprites/spritesheets/Inimigos/bluesheet2.png"); // obtido em: https://modelviewer.lol/

        if (!caminhoArquivoSpriteMovimento.empty()) {
            try {
                if (!getTextura().loadFromFile(caminhoArquivoSpriteMovimento))
                    throw std::runtime_error("Erro: não foi possível carregar a spritesheet do azulo em: " + caminhoArquivoSpriteMovimento);
    ...
    void Azulo::restaurarMemento(const Memento* memento) {
        ...
        const AzuloMemento* pMemento = dynamic_cast<const AzuloMemento*>(memento);
        if (pMemento) {
            tamanho = pMemento->tamanhoMemento;
        }
    }
}
